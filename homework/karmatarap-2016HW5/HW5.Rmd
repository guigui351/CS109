...
```{r}
# Had trouble assigning this directly to a variable, so using the default variable
...
```{r}
# The votes column is a categorical variable showing the voting choice 
#1 – Yes
#2 – Abstain
#3 – No
#8 – Absent
...
# Counts of each type of vote
votes %>% group_by(vote) %>% summarise(count=n())
...
**Your answer here**: 
A choice of 1,2 or 3, indicating a "yes", "no" or "abstention" respectively

Filter out the types of votes that do not provide information 
about our countries position on an issue from our dataset.

```{r}
# Keeping votes between 1 and 3 inclusive (yes,no,abstain)
votes <- votes %>% filter(vote %in% seq(1:3))
...
**Your answer here**:  The ccode column contains the COW (Correlates of War(numeric)) country codes.
...
```{r}
library(countrycode)
votes <- votes %>% mutate(country=countrycode(ccode,"cown","country.name"))
...
# Summarize votes by year
votes_per_year <- descriptions %>% mutate(year=year(date)) %>% group_by(year) %>% summarise(vpy=n())

# line plot of votes per year
library(ggplot2)
votes_per_year %>% filter(!is.na(year)) %>% ggplot(aes(year,vpy)) + geom_line() + xlab("Year") + ylab("Votes") + ggtitle("UN General Assembly Votes by Year")
...
votes_per_year <- votes_per_year %>% filter(vpy>1)
...
**Your answer here**:   U.S. State Department report Voting Practices in the United Nations

What percent of votes in history were categorized as "important"?

```{r}
# 6.88% of votes were classed as important
paste(round(100*(sum(descriptions$importantvote==1,na.rm=TRUE) / nrow(descriptions)),2),"%")
...
desc <- descriptions %>% select(-c(yes,no,abstain))

votes <- votes %>% left_join(desc,by=c("rcid","session"))
...
```{r}
# Removing years with NA
datx <-y %>% 
    filter(!is.na(year)) %>%
    group_by(year) %>% 
    summarize(agreement=mean(USA==Canada,na.rm=TRUE)*100)

# Basic linear regression
m <- datx %>% lm(agreement ~ year,data=.)
...
**Your answer here**: We are seeing a negative trend of agreement between the US and Canada over time. The trend is highly significant with a p-value <.001.
...
library(broom)
#I feel a span of between .2 and .25 creates the best fit
span <-.2

mod <- loess(agreement~year, degree=1, span = span, data=datx)
loess_fit <- augment(mod)


# Plotting the loess fit and highlighting the data corresponding to Reagan and Clinton administrations
datx %>% ggplot() +
  geom_point(aes(year, agreement)) + 
  geom_line(aes(x=year, y=.fitted), data=loess_fit, color="red") +
  annotate("rect", xmin=1992, xmax=2000, ymin=-Inf, ymax=Inf, alpha=.2, fill="blue") +
  annotate("rect", xmin=1980, xmax=1989, ymin=-Inf, ymax=Inf, alpha=.2, fill="green") +
  annotate("text", x=1996,y=100,label="Clinton") +
  annotate("text", x=1984,y=100,label="Reagan") +
  ylab("Agreement % (USA and Canada)") +
  ggtitle("UN Voting agreement between USA and Canada")


# We can see quite clearly here that during the Reagan administration, the voting agreement between the USA and Canada worsened significantly. The Clinton administration saw an overall improvement, but it was comparitively small change.

# In numbers, we see a decline of ~39% during the Reagan administration and a ~7% increase during the Clinton administration
datx%>% filter(year %in% c(1980,1989,1992,2000)) %>% mutate(administration=ifelse(year>1990,"Clinton","Reagan")) %>% group_by(administration) %>% summarise(change=last(agreement)-first(agreement))
...
# Select only countries of interest and extract year
country.votes <- votes %>% 
  filter(country %in% c("USA", "Israel","Mexico","Cuba","China","UK")) %>%
  mutate(year = year(date)) %>%
  select(rcid, year, importantvote, country, vote) 

# Compare US against each of the other countries
usa.votes.compare <- country.votes %>% 
  filter(country =="USA") %>% 
  mutate(USvote=vote) %>% 
  select(-c(country,vote)) %>% 
  left_join(country.votes %>% 
              select(rcid,country,vote) %>% 
              filter(country != "USA"),by="rcid") %>%
  group_by(country, year,importantvote) %>% 
  summarize(agreement= mean(USvote==vote, na.rm=TRUE)*100)

# Having a preliminary look through the using ggplot 
usa.votes.compare %>% 
  ggplot(aes(year, agreement,color=as.factor(importantvote),shape=as.factor(importantvote))) + 
  geom_point() + 
  stat_smooth(method="loess",span=span) +
  facet_wrap(~country) 


plotVotes <- function(important=TRUE){
  usa.votes.status <- usa.votes.compare %>% 
  filter(importantvote==important) %>% 
  group_by(country) %>% 
  do(augment(loess(agreement~year, degree=1, span = span, data=.),data=.))

  usa.votes.status %>% ggplot() +
  geom_point(aes(year, agreement)) + 
  geom_line(aes(x=year, y=.fitted), color="red") +
  facet_grid(~country) +
  ylab("% Agreement with USA") +
  ggtitle(paste("UN Voting agreement between USA and countries on",
                ifelse(important,"","Non -"), 
                "Important votes"))
}


# Checking agreement between US and each country votes on important issues
plotVotes()

# Repeat analysis for non-important countries
plotVotes(important = FALSE)
```

Describe the observed patterns.

**Your answer here**: On important issues, the US voting agreement with Israel is very strong. In many years, the US and Israel agree on every vote. In contrast, the US and Cuba disagree strongly. The disagreement with China is less strong, but every year, there is at most a 50% agreement record. The voting pattern with Mexico and the UK is less extreme, with Mexico mostly disagreeing and the UK mostly agreeing with the US.

For non-important issues, the voting pattern is more spread for each country except China. 
...
#Placing the distance into a matrix for interogating as the farthest is not clear from the dendogram
m <- as.matrix(d)


# USA is in row 75
which(m==max(m[75,]),arr.ind=T)
...
**Your answer here**: Closest is Israel, furthest is Cuba
...
```{r}
# Keep only important votes and countries of interest.
# Keeping variables of interest
countries.votes <- votes %>% 
  filter(importantvote==1 & country %in% c("USA", "Israel","Mexico","Cuba","China","UK")) %>%
  select(c(rcid,vote,country))

# Keeping only US yes or no votes.
# Renaming the US votes to y and joining it with the other country votes
# Transposing the countries so there is one record per voting session
usa.votes.agreement <- countries.votes %>%
  filter(country =="USA" & vote %in% c(1,3)) %>% 
  mutate(y=as.integer(vote==1)) %>% 
  select(-c(country,vote)) %>%
  left_join(countries.votes %>% 
              filter(country != "USA"),by="rcid") %>%
  spread(country, vote) %>%
  select(-rcid) #dropping rcid as its not needed anymore
  
# Filter out any rows with incomplete (NA) data as this causes problems in the knn model
usa.non.null.votes <- usa.votes.agreement[rowSums(is.na(usa.votes.agreement  ))==0,]
```
...
set.seed(1)
# Splitting the data 80/20 for training and testing respectively
inTrain <- createDataPartition(y = usa.non.null.votes$y,
                               p=0.8)
train_set <- slice(usa.non.null.votes, inTrain$Resample1)
test_set <- slice(usa.non.null.votes, -inTrain$Resample1)

# Fitting the GLM based only on the countries voting patterns
glm_fit <- glm(y~.,data=train_set)


f_hat <- round(predict(glm_fit, newdata = test_set, 
                 type = "response"))
...
```{r}
# Placing previous code into a function

runtestGLM <- function(indata=usa.non.null.votes) {
...
set.seed(1)
# Re-running 10 times
accuracy <- replicate(10,runtestGLM())

# Checking spread of results
summary(accuracy)

sd(accuracy)/sqrt(length(accuracy))
# We are not seeing that our model is accurate between 86 over 98% of the time on our limited sample with an se of .012
...
# Over the same training data we see a better accuracy than we observed in the glm. Even though our mean score matched what was observed in GLM, our se is much smaller at .007

set.seed(1)
# Create dataset splits for training and testing

# train with 20% of data, 10 splits
...
#Based on the final model k=4 was used
#Without using train, just testing to see if the prediction is similar
set.seed(1)
...
**Your answer here**: Small values of k seem to performt the best, with a marked drop in accuracy for k's > ~60. More specifically a value of k=4 seems to perform the best on our 10 samples giving a ~95% accuracy
